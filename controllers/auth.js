...
const transporter = nodemailer.createTransport(
  sendgridTransport({
    auth: {
      api_key:
        "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",
    },
  })
);
...
const register = async (req, res) => {
  ...
  transporter.sendMail({
    ...
    html: `<h4>Hi ${user.name}! Please click the below link for email confirmation.</h4>
      <p>Confirm email for <a href="https://guvi-task-session-38.herokuapp.com/api/v1/auth/confirm/${token}">${user.email}</a></p>
    `,
  });
...
const forgotPassword = async (req, res) => {
  ...
  transporter.sendMail({
    ...
    html: `<h4>Hi ${user.name}! Please click the below link to reset your password.</h4>
      <p>Confirm email for <a href="https://password-reset-task-38.netlify.app/reset-password/${user.email}">${user.email}</a></p>
    `,
  });
